...
class Database
{
    ...
    {
        $this->link = mysqli_connect("localhost","root","","fashion") or die ();
    ...
    public function countTableSQL($sql)
    {
        // $sql = "SELECT id FROM  {$table}";
        $result = mysqli_query($this->link, $sql) or die("Lỗi Truy Vấn countTable----" .mysqli_error($this->link));
        $num = mysqli_num_rows($result);
    ...
    public function fetchEmail($table , $email )
    {
        $sql = "SELECT * FROM {$table} WHERE email = '$email' ";
        $result = mysqli_query($this->link,$sql) or die("Lỗi  truy vấn fetchID " .mysqli_error($this->link));
        return mysqli_fetch_assoc($result);
    }
    public function fetchSub($table , $sub , $id_store)
    {   
        $sql = "SELECT *
                FROM category as c
                WHERE c.key = 0
    ...
    public function fetchAll($table, $id_store)
    {
        $sql = "SELECT * FROM {$table} WHERE id_store = '$id_store'" ;
        // _debug($sql);die;
        $result = mysqli_query($this->link,$sql) or die("Lỗi Truy Vấn fetchAll " .mysqli_error($this->link));
        $data = [];
        if( $result)
    ...
    public function orderBy($table, $id_store, $orderby)
    {
        $sql = "SELECT * FROM {$table} WHERE id_store = '$id_store'" ;
    ...
    public  function fetchJone($counttable,$sql ,$page = 0,$row ,$pagi = false )
    {
        //  Có thể bỏ $result = '';
        $result = '';
        $data = [];
